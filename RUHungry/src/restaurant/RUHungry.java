...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for(int i=0; i<categories; i++){
            ...
            for(int j=0; j<dishes; j++){
                ...
                Dish food = new Dish(categoryVar[i], dishName, stockID);
                MenuNode add = new MenuNode(food, front);
                front = add;
                menuVar[i] = front;
            }
            StdIn.readLine();
    ...
    public void addStockNode ( StockNode newNode ) {
        ...
        if(stockVar[index] == null){
            ...
            StockNode front = stockVar[index];
    ...
    public void deleteStockNode ( String ingredientName ) {
        ...
        StockNode match = null;
        StockNode prev = null;
        StockNode matchprev = null;
        int index = -1;
        for (int i=0; i<stockVar.length; i++){
            StockNode front = stockVar[i];
            while(front!=null && !(front.getIngredient().getName().equals(ingredientName))){
                prev = front;
                front = front.getNextStockNode();
            }
            if(front!=null){
                if(front.getIngredient().getName().equals(ingredientName)){
                    match = front;
                    matchprev = prev;
                    index = i;
                }
        ...
        if(match == stockVar[index]){
            if(match.getNextStockNode()!=null){
                stockVar[index] = match.getNextStockNode();
            }
            else{
                stockVar[index] = null;
            }
        ...
        else if(match.getNextStockNode() == null){
            matchprev.setNextStockNode(null);
        }

        else{
            matchprev.setNextStockNode(match.getNextStockNode());
        }
    }
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        while(ptr!=null && ptr.getIngredient().getID()!=ingredientID){
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if(ingredientName == null){
            stock = findStockNode(ingredientID);
        }
        else if(ingredientID == -1){
            stock = findStockNode(ingredientName);
        }

        if(stock!=null){
            int currentStock = stock.getIngredient().getStockLevel();
            stock.getIngredient().setStockLevel(currentStock + stockAmountToAdd);
        }
    }
    ...
    public void updatePriceAndProfit() {
        ...
        for(int i=0; i<menuVar.length; i++){
            MenuNode front = menuVar[i];
            while(front!=null){
                Dish d = front.getDish();
                double cost = 0;
                double price = 0;
                int[] stockID = d.getStockID();
                for(int j=0; j<stockID.length; j++){
                    int ID = stockID[j];
                    StockNode s = findStockNode(ID);
                    cost += s.getIngredient().getCost();
                }
                price = cost * 1.2;
                d.setPriceOfDish(price);
                d.setProfit(price-cost);
                front = front.getNextMenuNode();
            }
        }
    }
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        TransactionNode TN = new TransactionNode(data, null);
    ...
    public boolean checkDishAvailability ( String dishName, int numberOfDishes ){
        ...
        int[] ingredients = dish.getDish().getStockID();
        for(int i=0; i<ingredients.length; i++){
            int ingredientID = ingredients[i];
            StockNode stock = findStockNode(ingredientID);
    ...
    public void order ( String dishName, int quantity ){
        ...
        if(prepare){
            ...
            for(int i=0; i<ingredients.length; i++){
                int ingredientID = ingredients[i];
                int decrease = quantity*-1;
                updateStock(null, ingredientID, decrease);
            }
            ...
            addTransactionNode(data);
            MenuNode ptr = dish.getNextMenuNode();
            boolean possible = false;

            //category index
            String category = dish.getDish().getCategory();
            int index = -1;
            for(int i=0; i<categoryVar.length; i++){
                if(category.equals(categoryVar[i]))
                    index = i;
            }
            boolean atFront = false;
            if(dish==menuVar[index])
                atFront = true;
            ...
            while(ptr!=null && !possible && ptr!=dish){
                String newDishName = ptr.getDish().getDishName();
                possible = checkDishAvailability(newDishName, quantity);
                if(!possible){
                    TransactionData tran = new TransactionData("order", newDishName, quantity, 0, false);
                    addTransactionNode(tran);
                }
                if(possible){
                    ...
                    TransactionData tran = new TransactionData("order", newDishName, quantity, profit, true);
                    addTransactionNode(tran);
                }
                ptr = ptr.getNextMenuNode();
                if(ptr == null && !atFront){
    ...
    public void donation ( String ingredientName, int quantity ){
        ...
        if(profit>50 && stock.getIngredient().getStockLevel() >= quantity){
            success = true;
            int decrease = quantity*-1;
            updateStock(ingredientName, -1, decrease);
        }
    ...
    public void restock ( String ingredientName, int quantity ){
        ...
        double profit = profit();
        StockNode stock = findStockNode(ingredientName);
        double cost = stock.getIngredient().getCost() * quantity;
        if(profit >= cost){
            updateStock(ingredientName, -1, quantity);
            TransactionData restock = new TransactionData("restock", ingredientName, quantity, cost*-1, true);
            addTransactionNode(restock);
        }
        else{
            TransactionData restock = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(restock);
        }
    }
